...
class MarkdownExtra extends \Michelf\Markdown {
	...
	protected function _doAnchors_reference_callback($matches) {
		...
		if (isset($this->urls[$link_id])) {
			...
			$result .= "draggable=\"false\">$link_text</a>";
	...
	protected function _doImages_reference_callback($matches) {
		...
		if (isset($this->urls[$link_id])) {
			...
			$result = "<img class=\"post-image\" src=\"$url\" alt=\"$alt_text\"";
			if (isset($this->titles[$link_id])) {
				$title = $this->titles[$link_id];
				$title = $this->encodeAttribute($title);
				$result .=  " title=\"$title\"";
	...
	protected function _doFencedCodeBlocks_callback($matches) {
		...
		if ($classname !== "") {
			...
			$classes[] = $this->code_class_prefix . "hljs language-" . $classname;
		}
		$attr_str = $this->doExtraAttributes($this->code_attr_on_pre ? "pre" : "code", $attrs, null, $classes);
		$pre_attr_str  = $this->code_attr_on_pre ? $attr_str : '';
		$code_attr_str = $this->code_attr_on_pre ? '' : $attr_str;
		$codeblock  = "<pre class=\"code-block\"$pre_attr_str><code$code_attr_str>$codeblock</code></pre>";
		...
		while (!empty($this->footnotes_ordered)) {
			...
			if (!empty($this->fn_backlink_html)) {
				for ($ref_num = 1; $ref_num <= $ref_count; ++$ref_num) {
					...
					$backlink .= " <a href=\"#fnref$ref_count_mark:$note_id\"$parsed_attr draggable=\"false\">$backlink_text</a>";
				}
	...
	protected function _appendFootnotes_callback($matches) {
		...
		if (isset($this->footnotes[$node_id])) {
			...
			return
				...
				"<a href=\"#fn:$node_id\"$attr draggable=\"false\">$num</a>".
				"</sup>";
		}
